...
#   upw_evidence_bq_table <- bigrquery::bq_project_query("subugoe-closed", query = read_file("data_gathering_preprocessing/gather_oa_german_inst_upw_evidence.sql")) 
  upw_evidence <- bq_table_download(upw_evidence_bq_table, page_size = 20000)
...
We now classify the data with respect to their open access status according to the scheme from our [paper](https://doi.org/10.1007/s11192-021-04002-0) (Methodology section). As a preparatory step, we add ISSN_L information. The Unpaywall dataset already contains this information, however, since some records could not be matched to Unpaywall, we still perform this step.
...
# pubs_wos_upw_gold %>% 
#   filter(journal_is_oa == TRUE, issn_gold == FALSE) %>% 
#   select(`ISSN-L`) %>% 
#   distinct() %>% 
#   readr::write_csv("data/upw_gold_not_issn_list.csv")
```
...
We start out by gathering the OpenDOAR information. This is done by running [001_opendoar_api_call.R](data_gathering_preprocessing/opendoar_match/001_opendoar_api_call.R) and [002_tidy_opendoar_data.R](data_gathering_preprocessing/opendoar_match/002_tidy_opendoar_data.R). We continue by loading the gathered OpenDOAR informationand repository classification and matching it with our table of publications:
...
#pubs_wos_upw_cat <- readr::read_csv("data/pubs_cat_details.csv")
```
