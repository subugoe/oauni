...
sample_rows <- sample_rows %>% 
  select(upw_doi) %>% 
  distinct() %>% 
  left_join(pubs_wos_upw_cat, by = "upw_doi")
sample_rows %>% group_by(oa_category) %>% summarise(n = n_distinct(upw_doi)) %>% arrange(desc(n))
sample_rows %>% 
  select(upw_doi) %>% 
  distinct() %>% 
...
sample_rows %>% 
  ...
  distinct() %>% 
  readr::write_csv("data/sample_manual_check.csv")
```
