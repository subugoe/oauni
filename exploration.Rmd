...
pubs_cat_details %>% 
  ...
  select(n_total, PK_ITEMS, url_for_landing_page, url_for_pdf, evidence) %>% 
  distinct() %>% 
  group_by(PK_ITEMS, n_total) %>% 
  summarise(n_loc = n()) %>%  
  group_by(n_loc > 1, n_total) %>% 
  summarise(n_items = n()) %>% 
  mutate(prop = n_items/n_total)
...
```{r}
repo_df %>% 
  distinct(DOI, pdf_domain, landing_domain) %>% 
  left_join(opendoar,  by = c("pdf_domain" = "repo_domain", "landing_domain" = "repo_domain")) %>% 
  ungroup() %>% 
  group_by(repo_type) %>% 
  summarise(n = n()) %>% 
  arrange(desc(n)) %>% 
...
repo_df %>% 
  distinct(DOI, pdf_domain, landing_domain) %>% 
  filter(pdf_domain != "pdfs.semanticscholar.org") %>%
  left_join(opendoar,  by = c("pdf_domain" = "repo_domain", "landing_domain" = "repo_domain")) %>% 
  ungroup() %>% 
  group_by(repo_type) %>% 
  summarise(n = n()) %>% 
...
```{r}
repo_df %>% 
  distinct(pdf_domain, landing_domain) %>% 
  left_join(opendoar,  by = c("pdf_domain" = "repo_domain", "landing_domain" = "repo_domain"))

repo_df %>% 
  distinct(FK_ITEMS, DOI, pdf_domain, landing_domain) %>% 
  left_join(opendoar,  by = c("pdf_domain" = "repo_domain", "landing_domain" = "repo_domain")) %>% 
  ungroup() %>% 
...
pubs_cat %>% 
  inner_join(cr_publisher_full, by = c("DOI" = "doi")) %>% 
  filter(publisher == "Public Library of Science (PLoS)") %>% 
  group_by(PUBYEAR) %>% 
  summarise(n = n_distinct(DOI))

```
## Comparison Gold-list, Unpaywall
...
# pubs in both
pubs_cat_details %>%
  filter(host_type == "publisher") %>% 
  filter(issn_gold ==TRUE, journal_is_oa == TRUE) %>% 
  select(`ISSN-L`) %>% 
  distinct() -> both

pubs_cat_details %>%
  anti_join(both) %>%
  filter(issn_gold == TRUE & (host_type == "publisher"| is.na(host_type))) %>%
  select(`ISSN-L`) %>% 
  distinct() -> only_list
pubs_cat_details %>%
  anti_join(both) %>%
  filter(journal_is_oa == TRUE, host_type == "publisher") %>%
  select(`ISSN-L`) %>% 
  distinct() -> only_upw
```
