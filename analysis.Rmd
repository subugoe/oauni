...
pubs_cat %>%
  ...
  scale_color_viridis(
    ...
    labels = c("> 500,000", "> 50,000", "<= 50,000"),
    discrete = TRUE,
    direction = -1,
    option = "plasma"
  ) +
...
pubs_cat %>%
  ...
  geom_area(data = filter(pubs_all, oa_category == "is_oa"), aes(lubridate::ymd(paste0(PUBYEAR, "-01-01")), number_of_articles), fill = "#b3b3b3a0", color = "white", alpha = 0.2) +
...
```{r, fig.cap="OA shares per OA category for individual research institutions. The relationship between OA shares of individual OA categories (as per schema in Table 1) and overall OA share is shown."}
pubs_cat %>%
  mutate(oa_category = factor(
    oa_category,
    levels = c(
  ...
  ggplot(aes(x = (1 - not_oa), y = share, size = n_total, color = oa_route)) +
