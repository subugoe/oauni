...
host_overlap <- pubs_cat %>%
  ...
  mutate(oa_host = case_when(
    oa_host == "publisher" ~ "Publisher OA only",
    oa_host == "repository" ~ "Repository OA only",
    oa_host == "publisher&repository" ~ "Publisher & Repository OA"
  )) %>% 
  mutate(oa_host = factor(oa_host, levels = c("Publisher OA only", "Publisher & Repository OA", "Repository OA only"))) %>% 
...
pubs_cat %>%
  ...
  ggplot(aes(x = PUBYEAR, y = prop, group = oa_category)) +
    ...
    gghighlight::gghighlight(label_params = list(alpha = 0.8,
                                                 ...
                                                 segment.color = NA,
                                                 fontface = 'bold')) +
     geom_area(data = oa_shares_host_cat,
              aes(lubridate::ymd(paste0(PUBYEAR, "-01-01")), share_host, group = "none"),
...
for(secname in unique(pubs_cat$sec_abbr)){
  pubs_cat %>% 
    ...
    select(-not_oa) %>% 
    select(INST_NAME, n_total, is_oa, everything()) %>% 
    readr::write_csv(paste0("data/export_tables/table_oa_cat_inst_", secname,".csv"))
}
```
```{r}
#rm(oa_shares_inst, oa_shares_inst_sector, oa_shares_inst_sector_stats, oa_shares_sector, pubs_oa_year, pubs_oa_year_sec)
```
